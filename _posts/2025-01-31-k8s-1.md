...
안녕~ 이번 포스팅에서는 쿠버네티스(Kubernetes) 기초 내용을 쭉 정리해볼까 함. 지난번에 간단히 정리했던 포스팅에서 **주요 구성요소**, **오브젝트**, **kubectl 명령어 예시** 같은 것들을 좀 더 자세히 보강해달라는 요청이 있어서 이번에 내용을 추가했음. 내용이 좀 길지만, 차근히 읽어보면 도움이 될 거라 믿음!
...
1. **API Server (kube-apiserver)**
   ...
   ![How to Monitor Kubernetes API Server | Sysdig](https://sysdig.com/wp-content/uploads/how-to-monitor-kubernetes-apiserver-diagram.png)
...
## 6. 디플로이먼트(Deployment) & 레플리카셋(ReplicaSet)

- **ReplicaSet**: 특정 라벨(label)을 가진 파드를 `replicas`개만큼 유지함. (파드가 죽으면 새로 만들고, 많으면 줄이는 역할)
- **Deployment**: 실제로는 내부적으로 ReplicaSet을 사용하지만, 사용자가 직접 ReplicaSet을 관리하지 않고도 롤링 업데이트, 롤백, 버전 관리를 쉽게 하도록 해주는 상위 레벨 오브젝트.
...
## 9. 오브젝트 생성 & 관리하기 - `kubectl` 명령어 (자세히)
...
## 11. 노드 자원 보호와 유지보수 (drain & cordon)
